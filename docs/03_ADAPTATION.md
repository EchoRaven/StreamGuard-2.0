...
长程覆盖保证对**任意序列**成立，不需分布假设。**实测:gamma 默认值原来定错了。** 错误率在目标附近 3%↔8% 漂移、跑 3200 步:

| gamma | alpha_t 范围 | 饱和度 |
| --- | --- | --- |
| 0.002（现默认） | [0.027, 0.091] | **0.00** |
| 0.02（原默认） | [0.001, 0.500] | 0.07 |
| 0.08 | [0.001, 0.500] | 0.11 |

原默认 0.02 会让 alpha_t 摆满量程。经验法则:gamma 约取
**1/(一个漂移周期内的反馈条数)**。

⚠️ `clip` 是**诊断信号不是安全网**。持续撞边界说明 gamma 与漂移速率不
匹配——单向偏离累积到边界后 alpha_t 就再也不动,自适应失效而系统照跑
不报错。`AdaptiveConformal.saturation > 0.2` 即应换 gamma 或改用 DtACI。

⚠️ 符号极易写反。`err=1`(漏报)时要**放宽**。写反时表现为"错误率低于
目标反而放宽、高于目标反而收紧",且全程不报任何错。
